	...
	{#if childrenLinks.length === 0}
		<Collapsible.Trigger class="w-full {activeLink ? 'text-primary' : ''}">
			<a
				...
				class="flex w-full items-center gap-2 px-4 py-3 text-left hover:bg-muted popclick"
			>
				{#if Icon}
					<Icon size={18} />
				{/if}
		...
		<Collapsible.Trigger class="w-full {activeLink ? 'text-primary' : ''}">
			<span
				class="flex items-center gap-2 w-full justify-between px-4 py-3 text-left hover:bg-muted"
			>
